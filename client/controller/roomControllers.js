...
function connectSocket(){
    ...
    socket.on('message', (data) => {
        console.log(`[${data.userObject.pseudo}] ${data.content}`);
    });
    ...
    socket.on('roomCreated', (data) => {
        ...
        join13gemmesGame(globalRoomId);
    });
    socket.on('joinRoomValidation', (data) => {
        console.log(`Room rejoin : ${data.roomId}`);
        join13gemmesGame(globalRoomId);
    });
}

function createRoom(gameName, pseudo, gameParamsObject) {
    connectSocket();

    const roomId = "room-" + generateRoomId();
    globalRoomId = roomId;

    globalSocket.emit('createRoom', {roomId, pseudo, gameName, gameParamsObject});
}
